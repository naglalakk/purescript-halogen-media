...
component =
  ...
  handleAction = case _ of
    ...
    (MDOutput (MediaDisplay.RemovedMedia media)) -> H.raise $ Removed media
    (MDOutput (MediaDisplay.ScrollIsAtBottom)) -> H.raise $ ScrollIsAtBottom
    (ULOutput (Upload.UploadFiles files)) -> H.raise $ Upload files
    (ULOutput (Upload.DroppedFiles files)) -> H.raise $ Dropped files
    SwitchTab tab -> do
      H.modify_ _ { selectedTab = tab }
      H.raise $ TabSwitch tab
    InsertMedia -> do
      state <- H.get
      -- Clean up selected media
      H.modify_ _ { selectedMedia = [] }
      _ <- H.query (SProxy :: SProxy "mediaDisplay") unit (H.tell (MediaDisplay.Reset))
      H.raise $ InsertedMedia state.selectedMedia
    Receive inp -> do
      H.modify_
        _
          { selectedTab = fromMaybe DisplayTab inp.selectedTab
          , media = inp.media
          }
